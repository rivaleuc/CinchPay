...
function ensureStyles() {
    if (document.getElementById("cp-sdk-style"))
        return;
...
function buildUrl(opts, paymentId) {
    if (!opts.merchant)
        throw new Error("CinchPay: 'merchant' is required");
    ...
    if (opts.returnUrl)
        params.set("returnUrl", opts.returnUrl);
    if (opts.cancelUrl)
...
export function configure(config) {
    if (config.origin)
        state.origin = config.origin.replace(/\/$/, "");
}
/**
...
export function open(opts) {
    ...
    teardown();
    ensureStyles();
    const paymentId = (opts.paymentId ?? randomPaymentId());
    ...
    state.handler = (e) => {
        ...
        if (data.type === "cinchpay:ready") {
            try {
                opts.onReady?.(data.payload ?? { paymentId });
            }
            catch (err) {
                console.error(err);
            }
